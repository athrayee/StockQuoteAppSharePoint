...
var stockQuoteUrl1 = "//query.yahooapis.com/v1/public/yql?q=select%20Symbol%2CName%2CDaysLow%2CDaysHigh%2CYearLow%2CYearHigh%2CLastTradePriceOnly%2CLastTradeDate%2CLastTradeTime%2CMarketCapitalization%2CChange_PercentChange%20from%20yahoo.finance.quotes%20where%20symbol%20%3D%20%22";
var stockQuoteUrl2 = "%22%09&format=json&diagnostics=true&env=http%3A%2F%2Fdatatables.org%2Falltables.env&callback=JSON_CALLBACK";
//var url = window.location.protocol + "//" + window.location.host + _spPageContextInfo.siteServerRelativeUrl + "/";



var url = decodeURIComponent($.getQueryStringValue("SPAppWebUrl"));
...
stockquoteapp.service("StockQuoteService", function ($http) {
    ...
    this.GetStockPrice = function (callback) {
        ...
        $http.get(serverRootUrl, { headers: { 'Accept': 'application/json; odata=verbose' } }).success(function (data, status) {
            ...
            $http.jsonp(finalStockQuoteUrl).success(callback)
            {
...
angular.bootstrap(document.getElementById("stockquote"), ['stockquoteapp']);
